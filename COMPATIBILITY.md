...
The following are intentional v0.1 public surfaces:

- symbols listed in `vulnevidenceops.__all__`;
- the `vulnevidenceops` CLI commands `digest-json`, `schema` and `assess`;
- files matching `schemas/*.schema.json`;
- the internal control identities in `configs/control-evidence-matrix.json`.

Patch releases must remain backward compatible with the v0.1 contract. A future v1.0 release will
